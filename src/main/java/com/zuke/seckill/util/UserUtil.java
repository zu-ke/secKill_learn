...
public class UserUtil {
    public static void create(int count) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            User user = new User();
            user.setId(14725836902L + i);
            user.setNickname("user" + i);
            //小伙伴也可以生成不同的盐,这里老师就是有一个
            user.setSlat("33GoVAJW4tr5W5");//用户数据表的slat,由程序员设置
        ...
        String urlStr = "http://localhost:9999/login/doLogin";
        File file = new File("C:\\Users\\14200\\Desktop\\jmeter\\config.txt");
        if (file.exists()) {
            file.delete();
        }
